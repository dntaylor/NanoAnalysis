...
logger = logging.getLogger("MMJProcessor")
logging.basicConfig(level=logging.INFO, stream=sys.stderr,format='%(asctime)s.%(msecs)03d %(levelname)s %(name)s: %(message)s', datefmt='%Y-%m-%d %H:%M:%S')


class MMJProcessor(processor.ProcessorABC):
    ...
    def __init__(self,year='2018',corrections={}):
        ...
        mass_2_axis = hist.Bin("mass", r"$m_{2\ell j}$ [GeV]", 6000, 0, 1200)
        pt_axis = hist.Bin("pt", r"$p_{T,\ell}$ [GeV]", 3000, 0.25, 300)
        npvs_axis = hist.Bin("npvs", "Number of Vertices", 120, 0, 120)

        self._selections = ['iso','antiiso','jpsi_iso','jsi_antiiso']

        hist.Hist.DEFAULT_DTYPE = 'f'  # save some space by keeping float bin counts instead of double
        self._accumulator = processor.dict_accumulator()
        for sel in self._selections:
            self._accumulator['_'.join([sel, 'mmMass'])] = hist.Hist("Counts", dataset_axis, channel_axis, mass_axis)
            self._accumulator['_'.join([sel, 'mmjMass'])] = hist.Hist("Counts", dataset_axis, channel_axis, mass_2_axis)
    ...
    def process(self, df):
        ...
        passJets = (jets[jets.pt>20].counts >= 1)
        output['cutflow']['one jet'] += passJets.sum()
        selection.add('oneJet',passJets)
        ...
        passJPsi = (((mm_cands['p4'].mass>2.9) & (mm_cands['p4'].mass<3.1)).counts>0)
        output['cutflow']['mm jpsi'] += passJPsi.sum()
        selection.add('jpsi',passJPsi)

        # now add a jet (for now just the first)
        mmj_cands = mm_cands.cross(jets)
        mmj_cands = mmj_cands[mmj_cands['0']['p4'].delta_r(mmj_cands['1']['p4'])>0.8]
        mmj_cands = mmj_cands[:,:1]

        passMMJCand = (mmj_cands.counts>0)
        output['cutflow']['mmj cand'] += passMMJCand.sum()
        selection.add('mmjCand',passMMJCand)
        ...
        if self._isData: 
            ...
            for mi in range(2):
                ...
                eta = mm_cands[passMMJCand][mi]['p4'].eta.flatten()
                pt = mm_cands[passMMJCand][mi]['p4'].pt.flatten()
                if self._year == '2016':
                ...
                muonSF[passMMJCand] *= idSF
                muonSF[passMMJCand] *= isoSF

                weights.add('muonSF'+mi,muonSF)

        logging.debug('filling')
        for sel in self._selections:
            cutSels = ['lumiMask','trigger','goodVertex','twoLeptons','mmCand','oneJet','mmjCand']
            if sel=='iso':
                cut = selection.all(*cutSels+['iso'])
            elif sel=='antiiso':
                cut = selection.all(*cutSels+['antiiso'])
            elif sel=='jpsi_iso':
                cut = selection.all(*cutSels+['iso','jpsi'])
            elif sel=='jpsi_antiiso':
                cut = selection.all(*cutSels+['antiiso','jpsi'])
            chan = 'mmj'
            weight = weights.weight()*cut

            output['_'.join([sel,'mmMass'])].fill(
                dataset=dataset,
                channel=chan,
                mass=mm_cands[cut].p4.mass.flatten(),
                weight=weight[cut].flatten(),
            )
            output['_'.join([sel,'mmjMass'])].fill(
                dataset=dataset,
                channel=chan,
                mass=mmj_cands[cut].p4.mass.flatten(),
                weight=weight[cut].flatten(),
            )
...
if __name__ == '__main__':
    ...
    for year in years:
        ...
        processor_instance = MMJProcessor(
            year=year,
            corrections=corrections,
        )

        save(processor_instance, f'processors/mmjProcessor_{year}.coffea')
